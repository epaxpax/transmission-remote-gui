        ...
        return formStyled {
            ...
            Section(loc("Alternatív („turbó”) sebesség")) {
                ...
                if model.sessionInfo?.altSpeedTimeEnabled ?? false {
                    ...
                    SettingRow(title: "Napok", description: "Mely napokon lépjen életbe az időzített turbó.") {
                        DaySelector(mask: b.int(\.altSpeedTimeDay, default: 127) { $0.altSpeedTimeDay = $1 })
                    }
                }
            }
        }
...
/// Day-of-week selector for the alt-speed schedule. Transmission uses a bitmask
/// (bit 0 = Sunday … bit 6 = Saturday); shown Monday-first with localized labels.
    ...
    var body: some View {
        HStack(spacing: 3) {
            ...
            ForEach(order, id: \.self) { i in
                let bit = 1 << i
                let on = (mask & bit) != 0
                Button(labels.indices.contains(i) ? labels[i] : "?") {
