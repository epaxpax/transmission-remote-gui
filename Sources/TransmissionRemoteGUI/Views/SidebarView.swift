...
struct SidebarView: View {
    ...
    var body: some View {
        ...
        List {
            ...
            // Label (category) filters — only shown when torrents actually carry labels.
            if !model.allLabels.isEmpty {
                Section(loc("Címkék")) {
                    ForEach(model.allLabels, id: \.self) { label in
                        Button {
                            model.labelFilter = (model.labelFilter == label) ? nil : label   // toggle off if re-tapped
                        } label: {
                            HStack {
                        ...
                        .buttonStyle(.plain)
                        .listRowBackground(model.labelFilter == label ? Color.accentColor.opacity(0.18) : Color.clear)
                    }
                }
            }
        }
